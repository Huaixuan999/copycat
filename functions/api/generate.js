...
export async function onRequest(context) {
  ...
  if (request.method !== 'POST') {
    return new Response(JSON.stringify({ error: '只支持 POST 请求哦~' }), {
      status: 405,
      headers: corsHeaders,
    });
  }
  ...
  try {
    const { keywords, platform, tone, contentType } = await request.json();

    if (!keywords || !keywords.trim()) {
      return new Response(JSON.stringify({ error: '请输入关键词或主题哦~ 🐱' }), {
        status: 400,
        headers: corsHeaders,
      });
    }
